...
from attendee.test.factory import AttendeeEventFactory
...
class EventNestUserTestForUpdation(TestCase):
    ...
    def test_full_update_user(self):
        self.role = "organizer" if self.prev_user.role == "attendee" else "attendee"
        ...
        self.assertEqual(updated_data["organization"], serializer.data['organization'])
...
class EventsSerializerTest(TestCase):
    ...
    def test_valid_data(self):
        ...
        serializer_date = datetime.strptime(serializer.data['date'], '%Y-%m-%d').date()
        serializer_time = datetime.strptime(serializer.data['time'], '%H:%M:%S.%f').time()
        ...
        self.assertEqual(serializer.data['venue_details'], data['venue_details'])
...
class AttendanceSerializerTest(TestCase):
    def setUp(self):
        ...
        self.event_data = EventsFactory.create(registration_count=12, organization=self.organizer)
...
class AttendeeEventSerializerTest(TestCase):
    ...
    def setUp(self):
        ...
        self.registered_attendee = EventNestUserFactory.create_batch(10, role="attendee")
        self.attendee_event = [
        AttendeeEventFactory.create(Attendee=attendee, event=self.event)
    ...
    def test_get_registered_attendee_list(self):
        ...
        attendees = [attendee_event.Attendee for attendee_event in attendee_events]
        serializer = GetAttendeeSerializer(attendees, many=True)
        for attendee, registered_attendee in zip(serializer.data, self.registered_attendee):
...
class EventMediaFilesSerializerTest(TestCase):
    ...
    def test_event_media_files_deserialization(self):
        ...
        serializer = EventMediaFilesSerializer(instance=self.event, data=updated_data, partial=True)
